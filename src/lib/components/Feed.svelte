<script lang="ts">
  ...
  const eventVars = events.map(({ description }, i) => `--event-${i}: ${description}`).join('; ');
...
{#if $connectionLoaded}
  <ul class="text-lg text-gray-200 lg:max-w-8/12 lg:text-base xl:max-w-6/12" style={eventVars}>
    {#each events as event, i}
      {#if eventSwitches[i]}
        <li class="event mb-5" in:fade>
          {event.title} <br />
      ...
      {/if}
    {/each}
    <li class="event" in:fade={{ delay: 1500 * (events.length + 1) }}>
      <Heartbeat />
    </li>
  </ul>
{/if}
